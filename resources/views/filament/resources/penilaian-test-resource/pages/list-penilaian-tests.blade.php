<x-filament-panels::page>
    <div style="font-family: inherit;">
        ...
        <div style="display:flex; align-items:center; gap:8px; font-size:13px; color:#9ca3af; flex-wrap:wrap; margin-bottom:24px;">
            <button wire:click="backToMapel" style="font-weight:600;color:#6b7280;background:none;border:none;cursor:pointer;padding:0;" onmouseover="this.style.color='#2563eb'" onmouseout="this.style.color='#6b7280'">
                Mata Pelajaran
            </button>

            @if ($level !== 'mapel')
                <span style="color:#d1d5db;">/</span>
                <button wire:click="backToTab" style="font-weight:600;color:#6b7280;background:none;border:none;cursor:pointer;padding:0;" onmouseover="this.style.color='#2563eb'" onmouseout="this.style.color='#6b7280'">
                    {{ $selectedMapelLabel }}
                </button>
            @endif
            ...
            @if (in_array($level, ['slug', 'detail', 'siswa']))
                <span style="color:#d1d5db;">/</span>
                <button wire:click="backToSlug" style="font-weight:600;color:#6b7280;background:none;border:none;cursor:pointer;padding:0;" onmouseover="this.style.color='#2563eb'" onmouseout="this.style.color='#6b7280'">
                    {{ ucfirst($activeTab) }}
                </button>
            @endif
            ...
            @if (in_array($level, ['detail', 'siswa']))
                ...
                @if ($selectedSlugLabel)
                    ...
                    <span style="color:#d1d5db;">/</span>
                @endif
                <button wire:click="backToDetail" style="font-weight:600;color:#6b7280;background:none;border:none;cursor:pointer;padding:0;" onmouseover="this.style.color='#2563eb'" onmouseout="this.style.color='#6b7280'">
                    {{ $selectedLabel }}
                </button>
            @endif
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:16px;">
                ...
                @empty
                    <div style="grid-column:1/-1;background:#fff;border:1.5px solid #e5e7eb;border-radius:14px;padding:60px 20px;text-align:center;">
                        ...
                        <p style="color:#6b7280;font-weight:500;">Tidak ada mata pelajaran yang diajarkan.</p>
                    </div>
                @endforelse
            </div>
        @endif

        {{-- LEVEL 2-5: TAB + KONTEN --}}
                    ...
                    @forelse ($items as $item)
                        ...
                        >
                            <div style="display:flex;align-items:center;gap:16px;">
                                <div style="width:42px;height:42px;border-radius:12px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:{{ $activeTab === 'pretest' ? '#dbeafe' : ($activeTab === 'tugas' ? '#fef3c7' : '#dcfce7') }};color:{{ $activeTab === 'pretest' ? '#2563eb' : ($activeTab === 'tugas' ? '#d97706' : '#16a34a') }};">
                            ...
                            <div style="display:flex;align-items:center;gap:12px;flex-shrink:0;margin-left:16px;">
                                <span style="font-size:12px;font-weight:700;padding:4px 12px;border-radius:999px;background:{{ $item['jumlah'] > 0 ? '#dcfce7' : '#f3f4f6' }};color:{{ $item['jumlah'] > 0 ? '#16a34a' : '#9ca3af' }};">
                                    {{ $item['jumlah'] }} siswa
                                </span>
                                    ...
                                    <td style="padding:14px 20px;text-align:center;">
                                        @if ($activeTab === 'tugas')
                                            @if ($row['status'] === 'graded')
                                                <span style="display:inline-flex;align-items:center;gap:4px;padding:4px 12px;border-radius:999px;font-size:12px;font-weight:600;background:#dcfce7;color:#16a34a;">✓ Sudah Dinilai</span>
                                            @else
                                                <span style="display:inline-flex;align-items:center;gap:4px;padding:4px 12px;border-radius:999px;font-size:12px;font-weight:600;background:#fef3c7;color:#d97706;">⏳ Belum Dinilai</span>
                                            @endif
                                        @else
                                            @if ($row['status'])
                                                <span style="display:inline-flex;align-items:center;gap:4px;padding:4px 12px;border-radius:999px;font-size:12px;font-weight:600;background:#dcfce7;color:#16a34a;">✓ Lulus</span>
                                            @else
                                                <span style="display:inline-flex;align-items:center;gap:4px;padding:4px 12px;border-radius:999px;font-size:12px;font-weight:600;background:#fee2e2;color:#dc2626;">✗ Tidak Lulus</span>
                                            @endif
                                        @endif
                                    </td>
                                    <td style="padding:14px 20px;text-align:center;">
                            ...
                            @foreach ($data['soal'] as $no => $soal)
                                ...
                                <div style="background:{{ $bgColor }};border:1.5px solid {{ $borderColor }};border-radius:14px;padding:20px;">
                                    <div style="display:flex;align-items:flex-start;gap:14px;">
                                        <div style="width:32px;height:32px;border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:{{ $isBenar === null ? '#f3f4f6' : ($isBenar ? '#dcfce7' : '#fee2e2') }};font-weight:700;font-size:13px;color:{{ $isBenar === null ? '#6b7280' : ($isBenar ? '#16a34a' : '#dc2626') }};">
                                            {{ $no + 1 }}
                                        </div>
                                        <div style="flex:1;">
                                            ...
                                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px;">
                                                @foreach (['a' => $soal['opsi_a'], 'b' => $soal['opsi_b'], 'c' => $soal['opsi_c'], 'd' => $soal['opsi_d']] as $huruf => $opsi)
                                                    ...
                                                    <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:10px;background:{{ $opsBg }};border:1.5px solid {{ $opsBorder }};">
                                                        <span style="width:22px;height:22px;border-radius:6px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;background:{{ $isKunci ? '#16a34a' : ($isDipilih && !$isKunci ? '#dc2626' : '#e5e7eb') }};color:{{ ($isKunci || ($isDipilih && !$isKunci)) ? '#fff' : '#6b7280' }};">
                                                            {{ strtoupper($huruf) }}
                                                        </span>
