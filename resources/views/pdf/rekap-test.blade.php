...
<head>
    <meta charset="UTF-8">
    <title>Rekap Nilai Pre Test</title>
    <style>
        body {
...
<body>
    ...
    {{-- LOOP PER JUDUL PRETEST --}}
    @forelse($grouped as $pretestId => $rows)
        @php
            $pretest  = $rows->first()->pretest;
            $mapel    = $pretest?->slug?->schedule?->subject?->name ?? '-';
            $guru_row = $pretest?->slug?->schedule?->teacher?->name ?? '-';
            $tanggal  = $pretest?->waktu_mulai
                            ? \Carbon\Carbon::parse($pretest->waktu_mulai)->format('d-m-Y')
                            : '-';
        @endphp

        {{-- Judul tanpa garis bawah --}}
        <div style="font-weight: bold; font-size: 13px; margin-top: 20px; margin-bottom: 6px;">
            Pretest - {{ $pretest?->judul ?? '-' }}
        </div>
    ...
    @empty
        <p style="text-align:center; color:#888;">Tidak ada data pretest</p>
    @endforelse
